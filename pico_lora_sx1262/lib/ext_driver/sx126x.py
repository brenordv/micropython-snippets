...
class SX126X:
    ...
    def __init__(self, spi_bus, clk, mosi, miso, cs, irq, rst, gpio):
        ...
        if implementation.name == 'micropython':
            try:
                ...
                self.spi = SPI(spi_bus, baudrate=2000000, sck=Pin(clk), mosi=Pin(mosi),
                               miso=Pin(miso))  # Generic variant uPy
    ...
    def begin(self, bw, sf, cr, syncWord, currentLimit, preambleLength, tcxoVoltage, useRegulatorLDO=False, txIq=False,
              rxIq=False):
        self._bwKhz = bw
        self._sf = sf
    ...
    def beginFSK(self, br, freqDev, rxBw, currentLimit, preambleLength, dataShaping, preambleDetectorLength,
                 tcxoVoltage, useRegulatorLDO=False):
        self._br = 21333
        self._freqDev = 52428
    ...
    def scanChannel(self):
        ...
        state = self.setDioIrqParams(SX126X_IRQ_CAD_DETECTED | SX126X_IRQ_CAD_DONE,
    ...
    def startTransmit(self, data, len_, addr=0):
        ...
        if modem == SX126X_PACKET_TYPE_LORA:
            ...
            state = self.setPacketParamsFSK(self._preambleLengthFSK, self._crcTypeFSK, self._syncWordLength,
                                            self._addrComp, self._whitening, self._packetType, len_,
    ...
    def startReceive(self, timeout=SX126X_RX_TIMEOUT_INF):
        ...
        if modem == SX126X_PACKET_TYPE_LORA:
            ...
            state = self.setPacketParamsFSK(self._preambleLengthFSK, self._crcTypeFSK, self._syncWordLength,
                                            self._addrComp, self._whitening, self._packetType, self._packetLength,
                                            self._preambleDetectorLength)
        else:
            return ERR_UNKNOWN
        ASSERT(state)
    ...
    def startReceiveDutyCycleAuto(self, senderPreambleLength=0, minSymbols=8):
        ...
        wakePeriod = int(max((symbolLength * (senderPreambleLength + 1) - (sleepPeriod - 1000)) / 2,
    ...
    def startReceiveCommon(self):
        state = self.setDioIrqParams(
            SX126X_IRQ_RX_DONE | SX126X_IRQ_TIMEOUT | SX126X_IRQ_CRC_ERR | SX126X_IRQ_HEADER_ERR, SX126X_IRQ_RX_DONE)
        ASSERT(state)
        ...
        if modem == SX126X_PACKET_TYPE_LORA:
            ...
            state = self.setPacketParamsFSK(self._preambleLengthFSK, self._crcTypeFSK, self._syncWordLength,
                                            self._addrComp, self._whitening, self._packetType)
        else:
            return ERR_UNKNOWN
    ...
    def setSyncWord(self, syncWord, *args):
        if self.getPacketType() == SX126X_PACKET_TYPE_LORA:
            if len(args) > 0:
                ...
                controlBits = 0x44
            data = [int((syncWord & 0xF0) | ((controlBits & 0xF0) >> 4)),
    ...
    def setPreambleLength(self, preambleLength):
        ...
        if modem == SX126X_PACKET_TYPE_LORA:
            ...
            self._preambleLengthFSK = preambleLength
            return self.setPacketParamsFSK(self._preambleLengthFSK, self._crcTypeFSK, self._syncWordLength,
                                           self._addrComp, self._whitening, self._packetType, self._packetLength,
                                           self._preambleDetectorLength)
    ...
    def setSyncBits(self, syncWord, bitsLen):
        ...
        state = self.setPacketParamsFSK(self._preambleLengthFSK, self._crcTypeFSK, self._syncWordLength, self._addrComp,
                                        self._whitening, self._packetType, self._packetLength,
                                        self._preambleDetectorLength)
    ...
    def disableAddressFiltering(self):
        ...
        self._addrComp = SX126X_GFSK_ADDRESS_FILT_OFF
        return self.setPacketParamsFSK(self._preambleLengthFSK, self._crcTypeFSK, self._syncWordLength, self._addrComp,
                                       self._whitening, self._packetType, self._packetLength,
                                       self._preambleDetectorLength)
    ...
    def setPacketMode(self, mode, len_):
        ...
        state = self.setPacketParamsFSK(self._preambleLengthFSK, self._crcTypeFSK, self._syncWordLength, self._addrComp,
                                        self._whitening, mode, len_, self._preambleDetectorLength)
        ASSERT(state)
    ...
    def setPacketParamsFSK(self, preambleLength, crcType, syncWordLength, addrComp, whitening,
                           packetType=SX126X_GFSK_PACKET_VARIABLE, payloadLength=0xFF,
